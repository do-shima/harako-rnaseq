...
- Two conditions with two samples each is only the minimum computational DE
  gate. Small designs can still produce unstable estimates.
...
- QC-only normalized counts and TPM are descriptive. They are not substitutes
  for inferential differential-expression results.

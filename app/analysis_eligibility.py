...
def analysis_plan_from_rows(
    ...
    if not eligibility.structurally_valid:
        raise AnalysisPlanError(
            f"Cannot create analysis_plan for invalid sample rows: {eligibility.reason_code}"
        )
...
def assert_analysis_plan_consistent(
    ...
    expected_counts = {
        str(key): int(value)
        for key, value in dict(plan.get("condition_counts") or {}).items()
    }
    mismatches: list[str] = []
    expected_fields = {
    ...
    if mismatches:
        raise AnalysisPlanError(
            "Frozen analysis_plan does not match the frozen sample table: "
            + "; ".join(mismatches)
        )
    return actual
...
def resolve_analysis_plan(
    ...
    *,
    ...
    if not eligibility.structurally_valid:
        raise AnalysisPlanError(
            f"Sample table is structurally invalid: {eligibility.reason_code}"
        )
    if legacy_frozen and not eligibility.eligible_for_de:
        raise AnalysisPlanError(
            "Legacy frozen run is not eligible for differential expression under "
            "policy version 1. Create a new QC-only run; the existing run was not modified."
        )
